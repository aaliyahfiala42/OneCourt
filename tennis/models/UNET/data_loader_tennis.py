...
class BallUNetDataset(torch.utils.data.Dataset):
    def __init__(self, data_dir):
        ...
        self.inputs = sorted(glob.glob(data_dir + "/*/*/*.jpg"))
        labels = sorted(glob.glob(data_dir + "/*/*/*.csv"))
        
        #read in labels
        self.targets = []
        for i in range(len(labels)):
            #fill null values
            csv = pd.read_csv(labels[i]).fillna(value = 0)
    ...
    def __getitem__(self, idx):
        try:
            ...
            if ball_radius != 0:
                cv2.circle(target, (x, y), round(ball_radius), color=[255, 255, 255], thickness=-1)
            ...
            min_crop_u = max(0, x - rescaled_network_dim * 0.75)
            max_crop_u = min(x - rescaled_network_dim * 0.25, curr_input.shape[1] - rescaled_network_dim)
            start_crop_u = round(np.random.uniform(min_crop_u, max(min_crop_u, max_crop_u)))

            min_crop_v = max(0, y - rescaled_network_dim * 0.75)
            max_crop_v = min(y - rescaled_network_dim * 0.25, curr_input.shape[0] - rescaled_network_dim)
            ...
            new_ball_location = (x * scale_factor_x, y * scale_factor_y)
